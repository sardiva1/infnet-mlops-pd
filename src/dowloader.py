...
def list_remote_files(
    ...
    Args:
        dataset:        Kaggle dataset slug, e.g. 'shaistashahid/gaming-and-mental-health'.
    ...
    try:
        ...
        # lista de arquivos que desejamos. Unicode-safe: decode URL-encoded names
        matched = sorted(
            unquote(f.name) for f in files if fnmatch.fnmatch(unquote(f.name), file_pattern)
        )
        logger.info(
...
def download_dataset(
    ...
    Args:
        dataset:         Kaggle slug, e.g. 'shaistashahid/gaming-and-mental-health'.
    ...
    for filename in expected_files:
        ...
        if dest_path.exists():
            ...
            # Handle URL-encoded filenames from Kaggle API (e.g. %20 for spaces)
            candidates = list(destination_dir.glob(f"*{Path(filename).suffix}"))
            renamed = False
            for cand in candidates:
                if unquote(cand.name) == filename:
                    cand.rename(dest_path)
                    logger.info("Renamed downloaded file '%s' -> '%s'", cand.name, dest_path.name)
                    downloaded.append(dest_path)
                    renamed = True
                    break
