...
class MLflowTracker:
    ...
    def _configurar(self) -> None:
        ...
        if uri.startswith('sqlite:///'):
            # Extrai o caminho do arquivo SQLite (remove 'sqlite:///')
            db_path = uri[10:]  # Remove 'sqlite:///'
            if not Path(db_path).is_absolute():
                # Se é relativo, converte para absoluto baseado em root_dir
                db_path = str(self.root_dir / db_path)
            uri = f'sqlite:///{db_path}'
        elif not uri.startswith('sqlite:') and not uri.startswith('http'):
            # Caminho relativo (pasta mlruns) → converte para URI absoluta de pasta
    ...
    def registrar_modelo(self, run_id: str, registry_name: str) -> None:
        ...
        if self.logger:
            self.logger.info('Tentando registrar modelo: %s (URI: %s)', registry_name, model_uri)
        
        try:
            # Verifica se a run existe e tem artefatos
            run = mlflow.get_run(run_id)
            if self.logger:
                self.logger.debug('Run encontrado: %s', run_id)
            
            # Registra o modelo
            resultado = mlflow.register_model(model_uri=model_uri, name=registry_name)
            if self.logger:
                self.logger.info(
                    'Modelo registrado com sucesso: %s (versão %s)', 
                    registry_name, resultado.version
                )
        except mlflow.exceptions.MlflowException as mlflow_exc:
            if self.logger:
                self.logger.error(
                    'Erro MLflow ao registrar modelo: %s\n'
                    '  Run ID: %s\n'
                    '  Model URI: %s\n'
                    '  Mensagem: %s',
                    registry_name, run_id, model_uri, str(mlflow_exc)
                )
        except Exception as exc:
            if self.logger:
                self.logger.error(
                    'Erro inesperado ao registrar modelo: %s\n'
                    '  Run ID: %s\n'
                    '  Model URI: %s\n'
                    '  Erro: %s',
                    registry_name, run_id, model_uri, str(exc)
                )
