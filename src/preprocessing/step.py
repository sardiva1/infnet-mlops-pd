...
Implementa PipelineStep para a etapa de feature engineering:
  Entrada : data/processed/car_price.parquet  (gerado por ingestao + qualidade)
...
class PreprocessingStep(PipelineStep):
    ...
    def caminho_saida(self) -> Path:
        ...
        return output_dir / prep.get("output_filename", "car_price_features.parquet")
    ...
    def _transformar(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.listar_primeiras_linhas(df_out)

        return df_out

    def listar_primeiras_linhas(self, df: pd.DataFrame, n_linhas: int = 10) -> None:
        """
        Lista as n primeiras linhas do dataframe após o pré-processamento.
        
        Args:
            df (pd.DataFrame): Dataframe transformado após pré-processamento.
            n_linhas (int): Número de linhas para exibir (padrão: 10).
        """
        self.logger.info("-" * 60)
        self.logger.info("Primeiras %d linhas do dataset pré-processado:", n_linhas)
        self.logger.info("-" * 60)
        
        # Converter dataframe para string formatada
        df_head = df.head(n_linhas)
        df_str = df_head.to_string()
        
        # Logar cada linha para visualização
